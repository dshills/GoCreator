...
func (a *GoogleAdapter) Initialize(ctx context.Context) error {
	...
	opts := []googleai.Option{
		googleai.WithAPIKey(a.config.APIKey),
		googleai.WithDefaultModel(a.config.Model),
	}

	client, err := googleai.New(ctx, opts...)
	if err != nil {
		return providers.NewProviderError(a.id, providers.ErrorCodeAuth, "failed to initialize Google AI client", err)
	}
